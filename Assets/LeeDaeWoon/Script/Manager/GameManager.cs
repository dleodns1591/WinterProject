...
{
    public static GameManager instnace { get; private set; }
    ...
    private void Cheat_Key()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Keypad5))
            Entity_Manager.instance.Damage_Boss(true, 19);
        if (Input.GetKeyDown(KeyCode.Keypad6))
            Entity_Manager.instance.Damage_Boss(false, 19);
    }

    private void Start_Game()
    {
        // Turn_Manager에 있는 Start_GameCo를 실행시킨다.
        StartCoroutine(Turn_Manager.instance.Start_GameCo());
    }
    ...
    {
        Turn_Manager.instance.isLoading = true;
